...
public class RacingCarTest {
	...
	void 자동차_다음_동작에_따른_위치(CarNextStatus carNextStatus, int nextPosition){
		String name="exAud";
		RacingCar car = new RacingCar(name);
		car.changePosition(carNextStatus);
		assertThat(car.getPosition()).isEqualTo(nextPosition);
	}

	@ParameterizedTest
	@CsvSource(value={"0:Wait","3:Wait","4:Forward","9:Forward"},delimiter=':')
	void 난수에_따른_다음동작(int nextPosition, CarNextStatus carNextStatus){
		String name="exAud";
		RacingCar car = new RacingCar(name);
		CarNextStatus nextStatus = car.getNextStatus(nextPosition);
		assertThat(nextStatus).isEqualTo(carNextStatus);
